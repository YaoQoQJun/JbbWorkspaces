...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		function saveDataItem(){
			...
			$.ajax({
				type:"post",
				url:"hardwareStandard/addDataItem.do",
				data:{
				...
				success:function(msg){
					if(msg.result=="ok"){
						...
						var str="<li data-id='"+msg.dataItem_id+"'><a href='hardwareStandard/toHardwareStandard.do?dataItem_id="+msg.dataItem_id+"'>"+data_item_name+"</a></li>";
		...
		function updateDataItem(){
			...
			$.ajax({
				type:"post",
				url:"hardwareStandard/updateDataItem.do",
				data:{
					"data_item_name":data_item_name,
					"hbase_name":hbase_name,
					"exception_range":exception_range,
		...
		function saveData(){
			...
			$.ajax({
				type:"post",
				url:"hardwareStandard/addData.do",
				data:{
					"data_name":data_name,
		...
		function deleteData(data_id,e){
			...
			if(confirm("确认删除数据！")){
				...
				$.ajax({
					type:"post",
					url:"hardwareStandard/deleteData.do",
					async:false,
					data:{
						"data_id":data_id,
						},
        ...
        function refreshDataItem(){
        	window.location.href="<%=basePath%>hardwareStandard/toHardwareStandard.do";
        }
  ...
  <body class="pl15">
  	...
  	<div class="centMain bs-bmd" style="margin-top: 20px;">
        <span class="nasrnt">硬件标准数据设置</span>
    </div>
    <!-- 列表操作 -->
    <div class="centMain">
        ...
        <ul id="hblveitem" class="fl hblve mt15">
        	<c:forEach items="${dataItems}" var="dataItem">
      			<li data-id="${dataItem.id}"><a href="hardwareStandard/toHardwareStandard.do?dataItem_id=${dataItem.id}">${dataItem.data_item_name}</a></li>
      		</c:forEach>
        </ul>
        
        <div class="fl mt15 ml15">
       		<cc:ps privileges="${privileges}" privilege="增加,硬件标准数据">
        ...
        <div class="centMain fz15 mt20 pr15 pt15 brodc">
			...
			<cc:ps privileges="${privileges}" privilege="删除,硬件标准数据">
			<a class="sckre fr" onclick="removeDataItem(${dataItem.id},this);"><i class="icon icon-604"></i>删除</a>
			</cc:ps>
			<cc:ps privileges="${privileges}" privilege="修改,硬件标准数据">
