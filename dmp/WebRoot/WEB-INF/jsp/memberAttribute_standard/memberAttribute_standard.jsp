...
<html>
	...
	<script type="text/javascript">
		...
		function removeDataItem(id,e){
			if(confirm("确认删除数据项！")){
				$.ajax({
					type:"post",
					url:"hardwareStandard/deleteDataItem.do",
					async:false,
					data:{
						"data_item_id":id,
						},
					success:function(msg){
						if(msg=="ok"){
							...
							window.location.href="hardwareStandard/toHardwareStandard.do";
						}else{
							$.messager.show({
								title: "信息",
								msg: "删除数据项失败！"
		...
		function saveDataItem(){
			...
			$.ajax({
				type:"post",
				url:"memberAttributeStandard/addDataItem.do",
				data:{
				...
				success:function(msg){
					if(msg.result=="ok"){
						...
						var str="<li data-id='"+msg.dataItem_id+"'><a href='memberAttributeStandard/toMemberAttributeStandard.do?dataItem_id="+msg.dataItem_id+"'>"+data_item_name+"</a></li>";
						$("#hblveitem").append(str);
						
						$("#save_data_item_close_button").parents(".newmoent").addClass("display_none");
		...
		function updateDataItem(){
			...
			$.ajax({
				type:"post",
				url:"memberAttributeStandard/updateDataItem.do",
				data:{
					"data_item_name":data_item_name,
					"hbase_name":hbase_name,
					"exception_range":exception_range,
				...
				success:function(msg){
					if(msg=="ok"){
						...
						$("#update_data_item_close_button").parents(".newmoent").addClass("display_none");
		...
		function saveData(){
			...
			$.ajax({
				type:"post",
				url:"memberAttributeStandard/addData.do",
				data:{
					"data_name":data_name,
				...
				success:function(msg){
					if(msg.result=="ok"){
						...
						$("#save_data_close_button").parents(".newmoent").addClass("display_none");
		...
		function updateData(id){
			...
			$.ajax({
				type:"post",
				url:"memberAttributeStandard/updateData.do",
				data:{
					"data_name":data_name,
					"proportion":proportion,
					"id":data_id
		...
		function deleteData(data_id,e){
			...
			if(confirm("确认删除数据！")){
				...
				$.ajax({
					type:"post",
					url:"memberAttributeStandard/deleteData.do",
					async:false,
					data:{
						"data_id":data_id,
						},
        ...
        function refreshDataItem(){
        	window.location.href="<%=basePath%>memberAttributeStandard/toMemberAttributeStandard.do";
        }
  ...
  <body class="pl15">
  	...
  	<div class="centMain bs-bmd" style="margin-top: 20px;">
        <span class="nasrnt">属性标准数据设置</span>
    </div>
    <!-- 列表操作 -->
    <div class="centMain">
        ...
        <ul id="hblveitem" class="fl hblve mt15">
        	<c:forEach items="${dataItems}" var="dataItem">
      			<li data-id="${dataItem.id}"><a href="memberAttributeStandard/toMemberAttributeStandard.do?dataItem_id=${dataItem.id}">${dataItem.data_item_name}</a></li>
      		</c:forEach>
        </ul>
        
        <div class="fl mt15 ml15">
       		<cc:ps privileges="${privileges}" privilege="增加,属性标准数据">
        ...
        <div class="centMain fz15 mt20 pr15 pt15 brodc">
			...
			<cc:ps privileges="${privileges}" privilege="删除,属性标准数据">
			<a class="sckre fr" onclick="removeDataItem(${dataItem.id},this);"><i class="icon icon-604"></i>删除</a>
			</cc:ps>
			<cc:ps privileges="${privileges}" privilege="修改,属性标准数据">
	...
	<script type="text/javascript">
    	...
    	function ConsogleAvgimg(num){
    		...
    		var str1="{privilege_name=修改, model_name=硬件标准数据}";
    		var str2="{privilege_name=删除, model_name=硬件标准数据}";
